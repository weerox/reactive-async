...
private[rasync] trait CellCompleter[V, E >: Null] {
  ...
  /** Update `this` cells value with `x` and freeze it. The new value of `this` cell is determined
    * by its updater.
    */
  def putFinal(x: V): Unit

  /** Update `this` cells value with `x`. The new value of `this` cell is determined by its updater.
    */
  def putNext(x: V): Unit

  /** Update `this` cells value with `x`. If `isFinal` is `true`, the cell will be frozen. The new
    * value of `this` cell is determined by its updater.
    */
...
object CellCompleter {
  ...
  /** Create a completer for a cell holding values of type `V` given a `HandlerPool` and a `Key[V]`.
    */
  def apply[V, E >: Null](
      init: (Cell[V, E]) => Outcome[V] = (_: Cell[V, E]) => NoOutcome,
      sequential: Boolean = false,
      entity: E = null
  )(implicit updater: Updater[V], pool: HandlerPool[V, E]): CellCompleter[V, E] = {
    val impl =
  ...
  /** Create a cell completer which is already completed with value `result`.
    *
    * Note: there is no `K` type parameter, since we always use type `DefaultKey[V]`, no other key
    * would make sense.
    */
  def completed[V, E >: Null](result: V, entity: E = null)(implicit
      updater: Updater[V],
