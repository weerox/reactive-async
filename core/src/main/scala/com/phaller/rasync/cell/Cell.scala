...
trait Cell[V, E >: Null] {
  ...
  /** Returns the current value of `this` `Cell`.
    *
    * Note that this method may return non-deterministic values. To ensure deterministic executions
    * use the quiescence API of class `HandlerPool`.
    */
  def getResult(): V
  def getTry(): Try[V]
  def getState(): Try[ValueOutcome[V]]

  /** Start computations associated with this cell. The init method is called and relevant cells are
    * triggered recursivly.
    */
  def trigger(): Unit
  ...
  /** Adds a dependency on some `other` cell.
    ...
    *
    * @param valueCallback
    *   Callback that receives the new value of `other` and returns an `Outcome` for `this` cell.
    * @param other
    *   Cells that `this` Cell depends on.
    */
  def when(other: Iterable[Cell[V, E]])(
      valueCallback: Iterable[(Cell[V, E], Try[ValueOutcome[V]])] => Outcome[V]
  ): Unit

  // Convenience methods for one or two other cells
  def when(other: Cell[V, E])(
      valueCallback: Iterable[(Cell[V, E], Try[ValueOutcome[V]])] => Outcome[V]
  ): Unit
  def when(other1: Cell[V, E], other2: Cell[V, E])(
      valueCallback: Iterable[(Cell[V, E], Try[ValueOutcome[V]])] => Outcome[V]
  ): Unit
  ...
  private[rasync] def onNext[U](
      callback: Try[V] => U
  ): Unit // (implicit context: ExecutionContext): Unit
  ...
  /** Returns true, iff computations relevant to this cell are running. A cell becomes active, when
    * it is triggered via its `trigger` method, which in turn calls the `init` method. A cell
    * becomes inactive, when it is completed.
    */
  ...
  /** Put a final value to `this` cell, but do not propagate to some cells.
    ...
    *   The value to put.
    * @param dontCall
    *   The cells that won't be informed about the update.
    */
...
/** `CellImpl` uses a `State` to store the current value and dependency information. A state can
  * either be an `IntermediateState` or a `FinalState`.
  */
 ...
 */
private class IntermediateState[V, E >: Null](
    override val res: Success[V],
    val tasksActive: Boolean,
    val dependees: Map[Cell[V, E], CallbackRunnable[V, E]],
    val dependers: Set[Cell[V, E]]
) extends State[V]
...
/** Implementation of traits Cell and CellCompleter as the same run-time object.
  ...
  */
private[rasync] abstract class CellImpl[V, E >: Null](
    pool: HandlerPool[V, E],
    updater: Updater[V],
    override val init: (Cell[V, E]) => Outcome[V],
    override val entity: E
) extends Cell[V, E]
    with CellCompleter[V, E] {
   ...
   */
  private val state: AtomicReference[State[V]] = new AtomicReference(
    IntermediateState.empty[V, E](updater)
  )

  private val queuedCallbacks = new AtomicReference(
    new mutable.PriorityQueue[(Int, () => _)]()(SeqTaskOrdering)
  )
  ...
  override final def sequential(f: () => _, prio: Int): Unit = {
    ...
    if (queuedCallbacks.compareAndSet(currentQ, newQ)) {
      if (currentQ.isEmpty)
        startSequentialTask() // we could use the current thread for the first task. Refactor!
    } else sequential(f, prio)
  }

  /** Submit a queued sequential task to the handler pool. Continues submitting until the queue is
    * empty.
    */
  ...
  override final def when(
      other: Cell[V, E]
  )(valueCallback: Iterable[(Cell[V, E], Try[ValueOutcome[V]])] => Outcome[V]): Unit = {
    when(Iterable(other))(valueCallback)
  }

  override final def when(other1: Cell[V, E], other2: Cell[V, E])(
      valueCallback: Iterable[(Cell[V, E], Try[ValueOutcome[V]])] => Outcome[V]
  ): Unit = {
  ...
  @tailrec
  override final def when(other: Iterable[Cell[V, E]])(
      valueCallback: Iterable[(Cell[V, E], Try[ValueOutcome[V]])] => Outcome[V]
  ): Unit = state.get match {
    case _: FinalState[V] => // completed with final result
    ...
    case current: IntermediateState[V, E] => // not completed
      ...
      val newState = new IntermediateState[V, E](
        current.res,
        current.tasksActive,
        current.dependees ++ other.iterator.map(_ -> newCallback),
        current.dependers
      )
  ...
  @tailrec
  override final private[rasync] def addDependentCell(dependentCompleter: Cell[V, E]): Unit =
    state.get match {
      case _: FinalState[V] =>
        // call now!
      ...
      case current: IntermediateState[V, E] =>
        if (!current.dependers.contains(dependentCompleter)) { // ignore duplicates
          val newState = new IntermediateState[V, E](
            ...
            current.tasksActive,
            current.dependees,
            current.dependers + dependentCompleter
          )
          if (state.compareAndSet(current, newState)) {
  ...
  /** Called by 'putNext' and 'putFinal'. It will try to join the current state with the new value
    * by using the given updater and return the new value. If 'current == v' then it will return
    * 'v'.
    */
  ...
  /** Called by 'putNext' which will try creating a new state with some new value and then set the
    * new state. The function returns 'true' if it succeeds, 'false' if it fails.
    */
  @tailrec
  ...
  private[rasync] override def tryComplete(value: Try[V], dontCall: Option[Seq[Cell[V, E]]]): Unit =
    state.get match {
      case _: FinalState[V] => // completed with final result already
      // As decided by phaller, we ignore all updates after freeze and do not throw exceptions
  ...
  /** Remove `dependentCell` from the list of dependent cells. Afterwards, `dependentCell` won't get
    * informed about updates of `this` cell any more.
    */
  @tailrec
  override final private[rasync] def removeDependentCell(dependentCell: Cell[V, E]): Unit =
    state.get match {
      case current: IntermediateState[V, E] =>
  ...
  /** Remove `dependentCell` from the list of dependent cells. Afterwards, `dependentCell` won't get
    ...
    */
  @tailrec
  override private[rasync] final def removeDependeeCells(otherCells: Iterable[Cell[V, E]]): Unit =
    state.get match {
      case current: IntermediateState[V, E] =>
  ...
  /** Mark this cell as "running".
    *
    * @return
    *   Returns true, iff the cell's status changed (i.e. it had not been running before).
    */
  @tailrec
  override private[rasync] final def setTasksActive(): Boolean = state.get match {
    case current: IntermediateState[V, E] =>
      if (current.tasksActive)
        ...
        val newState =
          new IntermediateState(current.res, true, current.dependees, current.dependers)
...
private[rasync] class SequentialCellImpl[V, E >: Null](
    pool: HandlerPool[V, E],
    updater: Updater[V],
    override val init: (Cell[V, E]) => Outcome[V],
    override val entity: E
) extends CellImpl[V, E](pool, updater, init, entity)
    with SequentialCell[V, E]
private[rasync] class ConcurrentCellImpl[V, E >: Null](
    pool: HandlerPool[V, E],
    updater: Updater[V],
    override val init: (Cell[V, E]) => Outcome[V],
    override val entity: E
) extends CellImpl[V, E](pool, updater, init, entity)
    with ConcurrentCell[V, E]
