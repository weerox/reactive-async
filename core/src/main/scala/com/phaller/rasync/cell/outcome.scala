...
/** Use this trait in callbacks to return the new value of a cell. `NextOutcome(v)` and
  * `FinalOutcome(v)` put the value `v` into the cell; in the latter case the cell is completed. Use
  * `NoOutcome` to indicate that no progress is possible.
  */
...
final case class NextOutcome[+V](override val value: V)
    extends ValueOutcome[V]
    with IntermediateOutcome[V]
final case class FinalOutcome[+V](override val value: V)
    extends ValueOutcome[V]
    with FreezeOutcome[V]
...
object Outcome {
  ...
  /** Returns a `NextOutcome`, `FinalOutcome`, or `NoOutcome` object.
    *
    * If `valueOpt` is `None`, `NoOutcome` is returned. Otherwise, a `NextOutcome` or `FinalOutcome`
    * is returned depending on `isFinal`.
    *
    * @param valueOpt
    *   Option of a new value.
    * @param isFinal
    *   Indicates if the value is final.
    * @return
    *   Returns a `NextOutcome`, `FinalOutcome`, or `NoOutcome` object.
    */
  def apply[V](valueOpt: Option[V], isFinal: Boolean): Outcome[V] =
    valueOpt
      .map(value => if (isFinal) FinalOutcome(value) else NextOutcome(value))
      .getOrElse(NoOutcome)

  /** Returns a `NextOutcome`, `FinalOutcome`, or `NoOutcome` object.
    *
    * If `valueOpt` is `None`, `NoOutcome` is returned. Otherwise, a `NextOutcome` or `FinalOutcome`
    * is returned depending on the boolean parameter.
    *
    * @param valueOpt
    *   Option of a new value, a pair of a value V and a boolean to indicate if it is final.
    * @return
    *   Returns a `NextOutcome`, `FinalOutcome`, or `NoOutcome` object.
    */
