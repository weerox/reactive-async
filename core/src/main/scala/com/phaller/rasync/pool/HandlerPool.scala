...
class HandlerPool[V, E >: Null](
  ...
  private val virtualThreadFactory = 
    Thread
    .ofVirtual()
    .factory();
  ...
  /** Returns a new cell in this HandlerPool.
    *
    * Creates a new cell with the given key. The `init` method is used to determine an initial value
    * for that cell and to set up dependencies via `whenNext`. It gets called, when the cell is
    * awaited, either directly by the triggerExecution method of the HandlerPool or if a cell that
    * depends on this cell is awaited.
    *
    * @param init
    *   A callback to return the initial value for this cell and to set up dependencies.
    * @param updater
    *   The updater used to update the value of this cell.
    * @return
    *   Returns a cell.
    */
  def mkCell(init: (Cell[V, E]) => Outcome[V], entity: E = null)(implicit
      updater: Updater[V]
  ): Cell[V, E] = {
  ...
  def mkSequentialCell(init: (Cell[V, E]) => Outcome[V], entity: E = null)(implicit
      updater: Updater[V]
  ): Cell[V, E] = {
  ...
  /** Add an event handler that is called, when the pool reaches quiescence.
    *
    * The `handler` is called once after the pool reaches quiescence the first time after it has
    * been added.
    */
  @tailrec
  ...
  /** Remove all completed cells from cellsNotDone. Cells are not removed on deregister, but when
  ...
  /** Wait for a quiescent state. Afterwards, resolve all cells without dependencies with the
    * respective `fallback` value calculated by it's `Key`. Also, resolve cycles without
    * dependencies (cSCCs) using the respective `Key`'s `resolve` method. Both might lead to
    * computations on other cells being triggered. If more cells are unresolved, recursively wait
    * for resolution.
    *
    * @return
    *   The future is set once the resolve is finished and the quiescent state is reached. The
      ...
      val waitAgain: Boolean =
        if (independent.nonEmpty) {
          ...
          if (activeCells.isEmpty) {
            ...
            val cSCCs =
              closedSCCs[Cell[V, E]](activeCells, (cell: Cell[V, E]) => cell.cellDependencies)
            cSCCs
  ...
  private def resolve(
      cells: Iterable[Cell[V, E]],
      k: (Iterable[Cell[V, E]]) => Iterable[(Cell[V, E], V)]
  ): Boolean = {
    try {
      val results = k(cells)
  ...
  /** Increase the number of submitted tasks. Change the PoolState accordingly.
    */
  ...
  /** Decrease the number of submitted tasks and run registered handlers, if quiescent. Change the
    * PoolState accordingly.
    */
  ...
  /** If a cell is triggered, it's `init` method is run to both get an initial (or possibly final)
    * value and to set up dependencies (via whenNext/whenComplete). All dependees automatically get
    * triggered.
    *
    * @param cell
    *   The cell that is triggered.
    */
