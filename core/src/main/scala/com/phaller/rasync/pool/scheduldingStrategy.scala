...
/** A scheduling strategy defines priorities for dependency callbacks and cell resolution.
  *
  * Whenever a dependency is triggered due to a change of a dependee cell, a task is added to the
  * handler pool. Those tasks will eventually be picked up and executed, potentielly concurrently to
  * other tasks. Each task added to the pool is assigned a priority as defined by a
  * SchedulingStrategy, where tasks with lower priorities are more likely to be executed earlier.
  * (Note that due to concurrent execution there is no guarantee for any order of execution. Use
  * sequential callbacks to avoid concurrent execution of certain tasks.)
  *
  * If a tasks has been created because of a change in a dependee cell `other` being propgated to a
  * `dependentCell`, `SchedulingStrategy.calcPriority(dependentCell, other)` is called to obtain a
  * priority. If a tasks has been created to complete a `cell` via a `Key`,
...
class DefaultScheduling[V, E >: Null] extends SchedulingStrategy[V, E] {
  override def calcPriority(
      dependentCell: Cell[V, E],
      other: Cell[V, E],
      value: Try[ValueOutcome[V]]
  ): Int = 0
